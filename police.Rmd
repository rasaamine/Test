...
سال 2020 آمریکا شاهد اعتراضات عمومی به خشونت پلیس نسبت به سیاهاپوستان بود. اعتراضاتی که با کشته شدن جرج فلوید و انتشار عمومی فیلم لحظه مرگ او توسط پلیس به ایالت های مختلف گسترش یافت. بسیاری از اقلیت های نژادی اعتقاد دارند پلیس نسبت به آنان خشونت بیشتری اعمال می کند و نژادپرستی در رفتار ماموران پلیس عامل کشته شدن افراد بیگناه خصوصا در بین سیاهپوستان است. بر اساس داده هایی که روزنامه واشنگتن پست از افراد کشته شده بدست ماموران پلیس منتشر کرده می توان این موضوع را بر اساس آمار بررسی کرد.
</p>
</div>
```{r include=FALSE}
library(tidyverse)
...
Police_shooting %>% group_by(year, race2) %>%
  ...
  labs(fill = 'Race')+
  theme_minimal()+
  ggtitle ('Number of People Killed by Polic by Race')+
...
Police_shooting %>% ggplot(aes(age))+
  ...
  theme_minimal()+
  ggtitle ('Age distribution of people killed by police')+
  scale_fill_brewer(palette="Set3")
